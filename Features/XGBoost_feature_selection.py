...
# 根据XGBoost特征的重要性筛选特征
def select_features_by_XGBoost(train_data, drop_columns, label_name):
    ...
    for current_feature_importance in list(set(model.feature_importances_)):
        ...
        result = result.append(
            ...
            ,"nb_feature": [select_train_X.shape[1]]
            ,"accuracy_score": [accuracy_score(val_y, val_y_predict)]
        }))
    result = result.sort_values(by=["accuracy_score"], ascending=False)

    # 获取验证集精确率最高的特征重要性，并根据此重要性筛选特征
    best_threshold = result.iloc[0,0]
    return list(train_X.columns[model.feature_importances_>=best_threshold])
